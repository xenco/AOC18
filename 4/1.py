...
def main():
    ...
    guard = logbook.getSleepiestGuard()
    print("Guard %s slept a total of %s minutes, most at minute %s" % (
    guard.guard_id, guard.getSleepMinutesTotal(), guard.getMostSleptAtMinute()))
    print("Result: ", int(guard.guard_id) * guard.getMostSleptAtMinute())
